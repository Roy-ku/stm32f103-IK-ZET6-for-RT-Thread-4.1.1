...
void beep_set(void)
{
    ...
    // PWM设置恒为50%占空比方波
    rt_pwm_set(beep_data.pwm_dev, PKG_BEEP_PWM_DEV_CHANNEL, period, period >> 1);
}
#endif /* PKG_BEEP_PASSIVE_BUZZER */
...
#ifdef PKG_BEEP_PASSIVE_BUZZER
    RT_ASSERT(beep_data.pwm_dev);
    RT_ASSERT(freq == 0 || (freq >= PKG_BEEP_FREQ_MIN && freq <= PKG_BEEP_FREQ_MAX));
#endif
    RT_ASSERT(period >= PKG_BEEP_PERIOD_MIN && period <= PKG_BEEP_PERIOD_MAX);
...
void beep_on(void)
{
    ...
    rt_pwm_enable(beep_data.pwm_dev, PKG_BEEP_PWM_DEV_CHANNEL);
#else
...
void beep_off(void)
{
    ...
    rt_pwm_disable(beep_data.pwm_dev, PKG_BEEP_PWM_DEV_CHANNEL);
#else
...
void beep_thread_entry(void *parameter)
{
    ...
    {
#if defined(RT_USING_PM) && defined(PKG_BEEP_PASSIVE_BUZZER) && defined(PKG_BEEP_BLOCK_POWER_STOP)
        rt_pm_request(PKG_BEEP_REQUEST_PM_MODE);
#endif
        beep_on();
        rt_thread_mdelay(beep_data.pulse);

#if defined(RT_USING_PM) && defined(PKG_BEEP_PASSIVE_BUZZER) && defined(PKG_BEEP_BLOCK_POWER_STOP)
        rt_pm_release(PKG_BEEP_REQUEST_PM_MODE);
#endif
        beep_off();
        if (beep_data.npulse)
        {
...
static void __beep(rt_uint8_t argc, char **argv)
{
    ...
    else if (argc == 3)
    {
        nums = atoi(argv[1]);
        period = atoi(argv[2]);
        if (nums > 0 && period >= PKG_BEEP_PERIOD_MIN && period <= PKG_BEEP_PERIOD_MAX)
            beep(nums, period, 50, 0);
        else
            rt_kprintf("Out of range! Must be at: nums[1-any] period[%d-%d]\n", PKG_BEEP_PERIOD_MIN, PKG_BEEP_PERIOD_MAX);
    }
    else if (argc == 4)
    {
        nums = atoi(argv[1]);
    ...
    else
    {
        ...
        freq = atoi(argv[4]);
        if (nums > 0 && period >= PKG_BEEP_PERIOD_MIN && period <= PKG_BEEP_PERIOD_MAX
                && prcent > 0 && prcent <= 100 && (freq == 0 || (freq >= PKG_BEEP_FREQ_MIN && freq <= PKG_BEEP_FREQ_MAX)))
            beep(nums, period, prcent, freq);
        else
            rt_kprintf("Out of range! Must be at: nums[1-any] period[%d-%d] prcent[1-100] freq[0,%d-%d]\n", \
                       PKG_BEEP_PERIOD_MIN, PKG_BEEP_PERIOD_MAX, PKG_BEEP_FREQ_MIN, PKG_BEEP_FREQ_MAX);
    }
}
MSH_CMD_EXPORT_ALIAS(__beep, beep, beep <nums> <period> [prcent] [freq]);
