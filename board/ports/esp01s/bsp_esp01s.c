...
static rt_err_t bsp_uart_rx_ind(rt_device_t dev, rt_size_t size);
static char bsp_uart_get_char(void);
static void bsp_data_parsing_thread_entry(void *parameter);
...
static int bsp_esp01s_init(void)
{
    ...
    /* 設置接收回調函數 */
    rt_device_set_rx_indicate(esp01s_serial, bsp_uart_rx_ind);
    /* 發送字符串 */
    rt_device_write(esp01s_serial, 0, str, (sizeof(str) - 1));

    /* 創建 esp01s 線程 */
    if (rt_thread_init(&esp01s_thread, "esp01s", bsp_data_parsing_thread_entry, RT_NULL,
...
INIT_DEVICE_EXPORT(bsp_esp01s_init);
